...
cd $tmp_dir
wget -q https://github.com/Annopaolo/astartectl/releases/download/v22.11.01-test/astartectl_22.11.01-test_linux_x86_64.tar.gz
tar xf astartectl_22.11.01-test_linux_x86_64.tar.gz
chmod +x astartectl
cd -
...
astartectl cluster instances deploy --version "$1" --api-host "api.autotest.astarte-platform.org" --broker-host "broker.autotest.astarte-platform.org" \
    ...
    --cassandra-volume-size "4G" --name "astarte" --namespace "$2" -y --burst || exit 1
